...
std::vector<int> global_hist(NBINS, 0);
...
void setup_ui()
{
    ...
    sf::RenderWindow window(sf::VideoMode({WIDTH, HEIGHT}), "Burning Ship - MPI - SFML");
    ...
    while (window.isOpen())
    {
        ...
        std::vector<int> local_hist(NBINS, 0);
        burning_ship_mpi(x_min, y_min, x_max, y_max, WIDTH, HEIGHT, row_start, row_end,
                         max_iteraciones, pixel_buffer, local_hist.data());
        dibujar_texto(0);
        ...
        std::vector<int> all_hist(nprocs * NBINS);
        MPI_Gather(local_hist.data(), NBINS, MPI_INT, all_hist.data(), NBINS, MPI_INT, 0, MPI_COMM_WORLD);
        ...
        for (int r = 0; r < nprocs; r++)
        {
            for (int b = 0; b < NBINS; b++)
            {
                global_hist[b] += all_hist[r * NBINS + b];
            }
        }
...
int main(int argc, char **argv)
{
    ...
    else
    {
        while (true)
        {
            ...
            MPI_Gather(local_hist.data(), NBINS, MPI_INT, nullptr, NBINS, MPI_INT, 0, MPI_COMM_WORLD);
        }
    }
